...
public class WSUtil {
	...
	public static Mapping createMapping(PageContext pc, String componentPath) throws PageException, IOException {
		return CFMLEngineFactory.getInstance().getCreationUtil().createMapping(pc.getConfig(), "/ws", componentPath, null, Config.INSPECT_ONCE, true, false, false, false, true,
				true, null, -1, -1);
	}
	...
	public static Object broadcast(WebSocketEndpointFactory factory, Object res) throws PageException {
		if (res == null || res == NULL) return false;
		CFMLEngine eng = CFMLEngineFactory.getInstance();
		Cast cast = eng.getCastUtil();
		Decision dec = eng.getDecisionUtil();
	...
	public static Struct readConfig(PageContext pc, Log log) throws PageException, IOException {
		CFMLEngine eng = CFMLEngineFactory.getInstance();
		...
		if (!eng.getStringUtil().isEmpty(strConfigFile, true)) {
			strConfigFile = strConfigFile.trim();
			res = eng.getResourceUtil().toResourceExisting(pc, strConfigFile);
			info(log, "found system property/enviroment variable [lucee.websocket.config/LUCEE_WEBSOCKET_CONFIG] with value [" + strConfigFile + "]");
		}
		else {
		...
		if (!res.isFile()) {
			info(log, "no configuration found at  [" + res + "], using default settings");
			return eng.getCreationUtil().createStruct();
		}

		info(log, "found configuration at  [" + res + "]");

		String content = eng.getIOUtil().toString(res, eng.getCastUtil().toCharset("UTF-8"));

		return eng.getCastUtil().toStruct(deserializeJSON(pc, content));
	}
	...
	public static PageContext createPageContext(final ConfigWeb cw, final Session session, final String componentName) throws PageException, RuntimeException {
		...
		return createPageContext(cw, session, baos, "/" + componentName + ".cfc", session.getQueryString(), timeout);
	}
	...
	public static void trace(Log log, String msg, Throwable t) {
	...
	public static void info(Log log, String msg, Throwable t) {
	...
	public static Log getLog(Config config) {
		// TODO make sure we always Log to server context
